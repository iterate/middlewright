# middlewright demo video

A [Remotion](https://remotion.dev) project that renders the middlewright demo video — the story of a
...
pnpm render   # writes out/middlewright-demo.mp4
```

Structure:

- `src/Root.tsx` — scene list + durations (30fps, 1920×1080, ~74s)
- `src/scenes/` — one file per scene (title → setup → fail slow → sad path → middlewright → fail fast → add spinner → happy run → outro)
