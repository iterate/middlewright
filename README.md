...
Highlighted non-empty `fill()` actions reveal the completed field text from left to right during the rendered hold. The browser still runs one normal Playwright `fill()`; video mode captures the final field pixels and adds the typewriter-like effect in post. The surrounding frame stays at its pre-action state. If the field disappears or cannot be captured, rendering falls back to the ordinary stable highlight.
