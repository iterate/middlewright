...
`page.goto()` shows a Chrome-style address bar with the resolved destination URL, then removes it before the call returns. The default hold is 1000ms. Set `addressBar: { holdMs: 2000 }` to keep it readable for longer, or `addressBar: false` to disable it.
