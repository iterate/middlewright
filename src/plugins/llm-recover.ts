...
import type { Plugin, ActionContext } from "../plugin-system.ts";
...
function createAnthropicProvider(options: LlmRecoverOptions): RequestRecoveryCodeFn {
  ...
  if (!apiKey) {
    throw new Error(
      "[llm-recover] An Anthropic API key is required: pass `apiKey` or set the ANTHROPIC_API_KEY environment variable",
    );
  }
