...
- 2026-08-03: Isolated the text-cursor fill spec from the newly highlighted setup `waitFor()`. Its old boundary-pixel assertion now covers only the fill animation it was written to test.
- 2026-08-03: Moved that spec's white-background probe out of the pointer's top-left starting footprint. CI had consistently caught the pointer at `(30, 30)`; the bottom-left probe checks the same untouched background invariant.
