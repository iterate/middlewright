...
- Autofixes are limited to the requested shapes: argument-free `toBeVisible()` and string, template, or regular-expression `toContainText(expected)` calls. String `hasText` matching is case-insensitive, which is an intentional part of the requested rewrite.
...
- [x] Fix any remaining or malformed cases manually in a final implementation commit. *The dynamic eval assignment is kept with a narrow Oxlint suppression so statement-style generated functions remain valid.*
...
- 2026-08-04: Review follow-up parenthesizes complex locator receivers before adding methods and covers recovery functions with trailing semicolons.
